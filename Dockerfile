...
RUN \
	...
	&& git clone --branch ${RELEASE} --recurse-submodules=mozilla-ca https://github.com/MintNetwork/mint-blockchain.git . \
	&& /bin/sh ./install.sh
...
# copy build files
COPY --from=mint_build /mint-blockchain /mint-blockchain
